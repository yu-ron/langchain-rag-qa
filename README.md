...
- **RAG 全链路**：文档解析（PDF/Word/CSV/Markdown/TXT）→ 文本分割 → Embedding 向量化 → 检索 → LLM 流式生成
- **企业级检索优化**：
  - Query Rewriting 查询重写（多轮对话指代消解）
  - 混合检索（向量语义 + BM25 关键词 + RRF 融合排序）
  - Reranker 精排（LLM 逐条相关性打分，Top-12 粗召回 → Top-5 精排序）
- **引用溯源**：AI 回答标注 `[来源N]`，点击即可查看知识库原文，解决大模型"幻觉"问题
- **质量评估**：内置 RAGAS 风格评估脚本，量化检索相关性与答案忠实度
